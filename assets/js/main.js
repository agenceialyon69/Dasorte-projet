(function(){
  var burger=document.getElementById('burgerBtn'), menu=document.getElementById('mobileMenu'), closeBtn=document.getElementById('mobileClose');
  ...
  var floatCta=document.getElementById('floatCta'), heroEl=document.getElementById('top');
  if(floatCta){
    function toggleFloat(){
      var vh=window.innerHeight;
      if(heroEl){
        var heroBottom=heroEl.getBoundingClientRect().bottom;
        floatCta.classList.toggle('show', heroBottom<0);
      } else {
        floatCta.classList.toggle('show', window.scrollY>vh*0.6);
      }
    }
  ...
  // filters + gallery (portfolio page only)
  var filterBtns=document.querySelectorAll('.filter-btn');
  var figs=document.querySelectorAll('#gallery figure');
  if(filterBtns.length && figs.length){
  ...
  if(lb && lbImg && lbCap && lbClose && lbPrev && lbNext && figs.length){
    ...
    function show(i){
      var vf=visibleFigs(); if(!vf.length) return; current=(i+vf.length)%vf.length;
    ...
    function openLb(fig){
      ...
      lb.classList.add('open'); lbClose.focus(); document.body.style.overflow='hidden';
    }
  ...
  // form validation (book page only)
